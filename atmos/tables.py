...
# --- User-configurable table settings (km) ---
SI_START_KM = 0.0
SI_STOP_KM  = 86.0
SI_STEP_KM  = 0.1   # 0.1 km = 100 m

def LongUSTable():
    Itxt = open('us1py.output', 'w')
...
def LongSITable():
    ...
    for i in range(n + 1):
        ...
        Itxt.write(f"{altKm:6.2f} {sigma:8.4E} {delta:8.4E} {theta:6.4f}")
        temp = TZERO * theta
        pressure = PZERO * delta
        density = RHOZERO * sigma
        asound = AZERO * math.sqrt(theta)
        Itxt.write(f"{temp:6.1f} {pressure:8.3E} {density:8.3E} {asound:5.1f}")
        viscosity = MetricViscosity(theta)
        kinematicViscosity = viscosity / density
        Itxt.write(f"{1.0E6*viscosity:6.2f} {kinematicViscosity:8.2E}\n")

    Itxt.close()

def ShortSITable():
    Itxt = open('si2py.output', 'w')
    Itxt.write(' alt  sigma  delta  theta ')
    Itxt.write(' temp  press  dens   a    visc   k.visc ratio\n')
    Itxt.write('  Km                      ')
