{ This is the original program by Steven Pietrobon }
...
var H_,table1,table2,table3,table4{,table}:text;
    rho0,pi,Zinc:double;
    line:longint;
    ans:char;
...
var As,H,Z,P,Tm,T,M,rho,g:double;
    b,Zo,Zi,Zt:integer;
    P_,T_:temparray;
...
begin{lower atmosphere}
  ...
  for b := 0 to bmax-1 do
    begin{layers}
      ...
      if Lm[b] = 0.0
        then P_[b+1] := P_[b]*exp(-As*(H_[b+1]-H_[b])/T_[b])
        else P_[b+1] := P_[b]*exp(As*ln(T_[b]/T_[b+1])/Lm[b]);
      ...
      for Zi := Zo to Zt do
        begin{write table}
          ...
          Tm := T_[b] + Lm[b]*(H-H_[b]);
          if Lm[b] = 0.0
            then P := P_[b]*exp(-As*(H-H_[b])/T_[b])
            else P := P_[b]*exp(As*ln(T_[b]/Tm)/Lm[b]);
          M := mol(Z);
          T := Tm*M/M0;
          rho := (P/Tm)*(M0/Rs);
          g := gravity(Z);
